IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'K3_Entry_new' 
	   AND 	  type = 'P')
    DROP PROCEDURE K3_Entry_new
GO


--对应新镇街系统的存储过程
--本存储过程只能处理最多2个核算项目
CREATE  PROCEDURE K3_Entry_new(
	...
	begin
		...
		begin
			...
			begin
				...
				exec K_account_new @accountid, @unum, @name, @dc, @lv, @detail, null, null
			end
			else if (@detailcount = 1)
			begin
				print 'Count = 1' + convert(varchar, isnull(@ItemClassID2, 0)) + ';' + convert(varchar, isnull(@ItemClassID3, 0))
				exec K_account_new @accountid, @unum, @name, @dc, @lv, @detail, @ItemClassID2, null
			end
			else
			begin
				print 'Count = 2' + convert(varchar, isnull(@ItemClassID2, 0)) + ';' + convert(varchar, isnull(@ItemClassID3, 0))
				exec K_account_new @accountid, @unum, @name, @dc, @lv, @detail, @ItemClassID2, @ItemClassID3
			end
